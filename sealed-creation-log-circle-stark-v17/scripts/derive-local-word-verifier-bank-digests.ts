...
import { LOCAL_WORD_V16_VERIFIER_KEYS } from
  "../src/backends/circle/local-word-verifier-keys-v16.ts";
import {
  compileLocalWordVerifierBank,
  localWordVerifierBankDigest,
} from "../src/chain/local-word-role-manifest.ts";
import { LOCAL_WORD_V16_VERIFIER_BANK_DIGESTS_HEX } from
  "../src/chain/local-word-verifier-bank-digests-v16.ts";
 ...
 */
const rows = LOCAL_WORD_V16_VERIFIER_KEYS.map((key) => {
  ...
  const bank = compileLocalWordVerifierBank({
    profile: key.profile,
    proofBytes,
    constructionId: LOCAL_WORD_V16_CONSTRUCTION_ID,
  ...
  return {
    ...
    digestHex: Buffer.from(localWordVerifierBankDigest(bank, key.profile)).toString("hex"),
  };
});
...
const path = resolve(".local/local-word-verifier-bank-digests-v16.json");
