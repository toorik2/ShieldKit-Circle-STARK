 ...
 */
export const QM31_MUL_LIMB_ASM = `
${copy4(7, 6, 1, 0)}
${CM31_MUL_ASM}
OP_TOALTSTACK
OP_TOALTSTACK
...
/**
 * The same QM31 multiplication lowered through one native big-integer
 * convolution. Limbs are placed at bivariate exponents 0,1,3,4 in radix
 * B=2^64; every convolution coefficient is below B, so one OP_MUL recovers
 * the nine exact integer bilinear sums without carry overlap.
 *
 * This is arithmetic lowering, not a proof encoding: inputs and outputs remain
 * the canonical four M31 limbs used everywhere else.
 */
export const QM31_KRONECKER_BITS = 64;
export const QM31_PACK_KRONECKER_ASM = `
<${4 * QM31_KRONECKER_BITS}> OP_LSHIFTNUM
OP_SWAP
<${3 * QM31_KRONECKER_BITS}> OP_LSHIFTNUM
OP_ADD
OP_SWAP
<${QM31_KRONECKER_BITS}> OP_LSHIFTNUM
OP_ADD
OP_ADD
`;

/** Nine reduced convolution coefficients -> one reduced QM31 element. */
export const QM31_REDUCE_COEFFICIENTS_ASM = `
<4> OP_PICK OP_TOALTSTACK
<5> OP_PICK <4> OP_PICK
${M31_SUB}
OP_TOALTSTACK
<7> OP_PICK
...
/** 73-byte little-endian convolution blob -> one reduced QM31 element. */
export const QM31_REDUCE_KRONECKER_BLOB_ASM = `
${Array.from({ length: 9 }, () => `
<8> OP_SPLIT OP_SWAP <0x00> OP_CAT OP_BIN2NUM <${M31_P}> OP_MOD OP_SWAP
`).join("\n")}
OP_DROP
${QM31_REDUCE_COEFFICIENTS_ASM}`;

export const QM31_MUL_KRONECKER_ASM = `
${QM31_PACK_KRONECKER_ASM}
OP_TOALTSTACK
${QM31_PACK_KRONECKER_ASM}
OP_FROMALTSTACK
OP_MUL
<73> OP_NUM2BIN
${QM31_REDUCE_KRONECKER_BLOB_ASM}`;

/** Canonical BCH-2026 lowering used by all verifier roles. */
export const QM31_MUL_ASM = QM31_MUL_KRONECKER_ASM;
...
export function compileQm31MulKroneckerLock(
  expected: readonly [bigint, bigint, bigint, bigint],
): Uint8Array {
  const bin = cashAssemblyToBin(expectQm31Asm(QM31_MUL_KRONECKER_ASM, expected));
  if (typeof bin === "string") throw new Error(`qm31-mul-kronecker: ${bin}`);
  return bin;
}
