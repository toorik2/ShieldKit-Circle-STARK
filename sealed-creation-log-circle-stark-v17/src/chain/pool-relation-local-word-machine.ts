...
import type { RelationCreation, RelationMembership, RelationWitness } from "../pool/relation-witness.ts";
import { readU16LE, writeI64LE, writeU64BE } from "../pool/bytes.ts";
import {
...
import { LOCAL_WORD_PRODUCTION_PARAMETERS } from "../backends/circle/local-word-successor-params.ts";
import {
...
export type PoolLocalShaPublicField = "statement-digest";
...
export type PoolLocalShaGraph = {
  ...
  /** One frozen product domain across all profiles; active rows remain profile-minimal. */
...
type CreationWires = {
  readonly nextHead: readonly number[];
  readonly edge: readonly number[];
};

type RelationStatementWires = {
  readonly reserveMovement: readonly number[];
  readonly category: readonly number[];
  readonly oldCreationIndex: readonly number[];
  readonly oldCreationHead: readonly number[];
  readonly oldHistoryRoot: readonly number[];
  readonly newCreationHead: readonly number[];
  readonly createdEdge: readonly number[];
  readonly nullifier: readonly number[];
};
...
function creationIndexBytes(index: bigint, label: string): Uint8Array {
  if (index < 0n || index >= 1n << BigInt(EDGE_HISTORY_DEPTH)) {
    throw new Error(`${label} creation index`);
  }
  return writeU64BE(index);
}
...
class PoolLocalShaCompiler {
  ...
  /**
   * One SHA-256 commitment binds the complete public relation statement. The
   * relation consumes slices of this same private preimage, so there is no
   * second field-by-field public doorway.
   */
  private statement(statement: PoolStatement): RelationStatementWires {
    const encoded = encodeLocalWordRelationStatement(statement, this.minerFeeSats);
    const words = this.privateWords(encoded, "relation-statement");
    if (words.length !== LOCAL_WORD_RELATION_STATEMENT_BYTES / 4) {
      throw new Error("relation statement word geometry");
    }
    const digest = this.hash(words, LOCAL_WORD_RELATION_STATEMENT_BYTES, "relation-statement:digest");
    this.bind(
      digest,
      localWordRelationStatementDigest(statement, this.minerFeeSats),
      "statement-digest",
    );
    const slice = (start: number, width: number, label: string): readonly number[] => {
      const selected = words.slice(start, start + width);
      if (selected.length !== width) throw new Error(`${label} relation statement slice`);
      return selected;
    };
    return {
      reserveMovement: slice(3, 2, "reserve movement"),
      category: slice(7, 8, "category"),
      oldCreationIndex: slice(19, 2, "old creation index"),
      oldCreationHead: slice(23, 8, "old creation head"),
      oldHistoryRoot: slice(31, 8, "old history root"),
      newCreationHead: slice(55, 8, "new creation head"),
      createdEdge: slice(79, 8, "created edge"),
      nullifier: slice(87, 8, "nullifier"),
    };
  }
  ...
  /** Expand one authenticated index bit to an all-zero or all-one word. */
  private bitMask(source: number, bit: number, label: string): number {
    if (!Number.isInteger(bit) || bit < 0 || bit >= 32) throw new Error(`${label} bit`);
    let mask = this.builder.and(source, this.builder.constant((2 ** bit) >>> 0), `${label}:isolate`);
    if (bit !== 0) mask = this.builder.rotate(mask, bit, `${label}:normalize`);
    for (const shift of [31, 30, 28, 24, 16]) {
      mask = this.builder.or(mask, this.builder.rotate(mask, shift, `${label}:spread:${shift}`), `${label}:merge:${shift}`);
    }
    return mask;
  }

  /**
   * The non-self-referential relation context is fixed by the v16 relation.
   * The proof construction identifier separately binds this compiled graph.
   */
  private creation(
    noteCommitment: readonly number[],
    category: readonly number[],
    index: readonly number[],
    previousHead: readonly number[],
    label: string,
  ): CreationWires {
    if (noteCommitment.length !== 8 || category.length !== 8 || index.length !== 2 || previousHead.length !== 8) {
      throw new Error(`${label} creation geometry`);
    }
    const nextHead = this.hash([
      ...this.constantWords(CREATION_LINK_DOMAIN),
      ...this.constantWords(CREATION_LOG_CONTEXT),
      ...category,
      ...index,
      ...previousHead,
      ...noteCommitment,
    ], 168, `${label}:link`);
    const edge = this.hash([
      ...this.constantWords(CREATION_EDGE_DOMAIN),
      ...this.constantWords(CREATION_LOG_CONTEXT),
      ...category,
      ...index,
      ...previousHead,
    ...
    return { nextHead, edge };
  }

  private currentCreation(
    noteCommitment: readonly number[],
    statement: RelationStatementWires,
    label: string,
  ): CreationWires {
    const creation = this.creation(
      noteCommitment,
      statement.category,
      statement.oldCreationIndex,
      statement.oldCreationHead,
      label,
    );
    this.builder.equalWords(creation.nextHead, statement.newCreationHead);
    this.builder.equalWords(creation.edge, statement.createdEdge);
    return creation;
  }
  ...
  private pathSources(
    ...
    if (path.length !== EDGE_HISTORY_DEPTH) throw new Error(`${label} edge path shape`);
    const siblings = path.map((sibling, level) => {
      assertWidth(sibling, 32, `${label} sibling ${level}`);
      return this.privateWords(sibling, `${label}:sibling:${level}`);
    });
    const directions = path.map((_, level) => this.bitMask(indexLowWord, level, `${label}:direction:${level}`));
    return { siblings, directions };
  }

  private historyMembership(
    edge: readonly number[],
    membership: RelationMembership,
    index: readonly number[],
    historyRoot: readonly number[],
    label: string,
  ): void {
    if (index.length !== 2) throw new Error(`${label} index geometry`);
    const sources = this.pathSources(membership.path, index[1]!, label);
    let accumulator = edge;
    ...
    this.builder.equalWords(accumulator, historyRoot);
  }

  private historicalCreation(
    noteCommitment: readonly number[],
    membership: RelationMembership,
    category: readonly number[],
    label: string,
  ): CreationWires & { readonly index: readonly number[] } {
    assertWidth(membership.previousHead, 32, `${label} previous head`);
    const index = this.privateWords(creationIndexBytes(membership.creationIndex, label), `${label}:index`);
    // The history capacity is 2^32, so the canonical u64 encoding has a zero high word.
    this.builder.equalWord(index[0]!, this.builder.constant(0));
    const previousHead = this.privateWords(membership.previousHead, `${label}:previous-head`);
    return { ...this.creation(noteCommitment, category, index, previousHead, label), index };
  }

  deposit(statement: PoolStatement, created: Note): PoolLocalShaAmountWires {
    const relationStatement = this.statement(statement);
    const note = this.note(created, "deposit-note");
    this.builder.equalWords(note.amount, relationStatement.reserveMovement);
    this.currentCreation(note.leaf, relationStatement, "deposit-creation");
  ...
  withdraw(
    ...
    const withdrawalWords = relationStatement.reserveMovement;
    const historical = this.historicalCreation(
      note.leaf,
      spent,
      relationStatement.category,
      "spent-creation",
    );
    this.historyMembership(
      historical.edge,
      spent,
      historical.index,
      relationStatement.oldHistoryRoot,
      "spent-edge-membership",
    );
    const nullifier = this.hash([
      ...this.constantWords(EDGE_NULLIFIER_DOMAIN),
      ...this.constantWords(CREATION_LOG_CONTEXT),
      ...relationStatement.category,
      ...historical.edge,
      ...note.owner,
      ...note.rho,
    ], 192, "spent-note:nullifier");
    this.builder.equalWords(nullifier, relationStatement.nullifier);

    if (!change) {
      this.builder.equalWords(note.amount, withdrawalWords);
    ...
    // allocation remain private; only its linked creation head and edge are public.
    const changed = this.note(change.note, "change-note", note.owner);
    this.currentCreation(changed.leaf, relationStatement, "change-creation");
...
/** Compile the complete private SHA and sealed-creation-log portion of pool relation v1. */
export function compilePoolLocalShaGraph(
  ...
  if (statement.action === "DEPOSIT") {
    ...
    return compiler.finish(compiler.deposit(statement, witness.created.note), "deposit");
  }
