...
import { LOCAL_WORD_RELATION_CONSTRUCTION_VERSION } from
  "./local-word-relation-version.ts";
import type {
  PoolLocalShaGraph,
...
export { LOCAL_WORD_RELATION_CONSTRUCTION_VERSION } from
  "./local-word-relation-version.ts";
...
const PUBLIC_FIELD_ID: Readonly<Record<PoolLocalShaPublicField, number>> = {
  // IDs 0..8 belonged to superseded public amount/note fields.
  "statement-digest": 9,
};
...
export function encodePoolLocalShaConstruction(graph: PoolLocalShaGraph): Uint8Array {
  ...
  if (!Number.isSafeInteger(graph.relationRows) ||
    graph.relationRows < geometry.relationRows ||
    (graph.relationRows & (graph.relationRows - 1)) !== 0) {
    throw new Error("pool local SHA frozen relation rows");
  }
  const out: number[] = [
    ...CONSTRUCTION_MAGIC,
