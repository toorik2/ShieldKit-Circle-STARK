...
type BaseLayout = {
  ...
  readonly quotient?: Qm31Positions;
  readonly zerofier?: number;
};
...
class AirAssembly {
  ...
  readQm31(
    base: "interaction" | "globalCurrent" | "globalPrevious" | "challenges" |
      "alpha" | "claimedSum" | "quotient",
    byteOffset: number,
  ): void {
    const value: Qm31Positions | undefined = base === "alpha" || base === "claimedSum" ||
      base === "quotient"
      ? this.bases[base]
  ...
  readZerofier(): void {
    if (this.bases.zerofier === undefined) throw new Error("local-word AIR missing zerofier");
    this.emit(`<${this.depth(this.bases.zerofier)}> OP_PICK`);
    this.extra += 1;
  }
...
export type V17LocalWordAirQuotientGateArgs = {
...
function localWordZerofierTable(parameters: LocalWordProofParameters): Uint8Array {
...
/** Evaluate the canonical AIR and feed its result directly into the quotient. */
export function compileV17LocalWordAirQuotientGate(
  args: V17LocalWordAirQuotientGateArgs,
): Uint8Array {
  ...
  const quotientAndFriMask = LOCAL_WORD_MATRIX_NAMES.indexOf("quotientAndFriMask");
  ...
  const claimedSumPositions = appendStaticQm31(offsets.publicClaimedSum, 1)[0]!;
  const quotientPositions = appendQm31Row({
  ...
  const orbitCount = 2 ** (parameters.evalLog - parameters.relationLog);
  const table = localWordZerofierTable(parameters);
  setup.push(`${staticBlobAssembly(baseCount, offsets.queries + args.query * 4, 4)}
OP_REVERSEBYTES OP_BIN2NUM <${parameters.relationLog}> OP_RSHIFTNUM
OP_DUP OP_0 OP_GREATERTHANOREQUAL OP_VERIFY
OP_DUP <${orbitCount}> OP_LESSTHAN OP_VERIFY
<4> OP_MUL <0x${hex(table)}> OP_SWAP OP_SPLIT OP_NIP
<4> OP_SPLIT OP_DROP OP_BIN2NUM`);
  const zerofierPosition = baseCount;
  baseCount += 1;
  const bases: BaseLayout = {
    ...
    quotient: quotientPositions,
    zerofier: zerofierPosition,
  };
  ...
  for (let residual = LOCAL_WORD_AIR_CONSTRAINTS - 1; residual >= 0; residual -= 1) {
    ...
    air.qmAdd();
    if (air.extra !== 0) throw new Error(`v17 direct AIR Horner stack ${residual}:${air.extra}`);
  }
  air.readQm31("quotient", 0);
  air.readZerofier();
  air.qmMulM31();
  air.qmSub();
  if (air.extra !== 0) throw new Error(`v17 direct AIR quotient stack ${air.extra}`);
  const definitions = [
    define(localWordReadDynamicAssembly(args.reader), READ_BYTES, "local-word AIR reader"),
...
OP_1`, `v17 direct AIR quotient ${args.query}`);
}
