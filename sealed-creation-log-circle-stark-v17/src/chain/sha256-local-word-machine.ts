...
export function localShaGeometry(
  ...
  frozenRelationRows = localShaRelationRows(program),
): LocalShaGeometry {
  const minimumRelationRows = localShaRelationRows(program);
  if (!Number.isSafeInteger(frozenRelationRows) || frozenRelationRows < minimumRelationRows ||
    (frozenRelationRows & (frozenRelationRows - 1)) !== 0) {
    throw new Error("local SHA frozen relation rows");
  }
  const relationRows = frozenRelationRows;
