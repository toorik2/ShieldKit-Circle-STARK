...
export const NULLIFIER_USED_TAG = new TextEncoder().encode("ShieldKit/PAA2/nullifier-used/v1");
...
export class SparseNullifierTree {
  ...
  clone(): SparseNullifierTree {
    const copy = new SparseNullifierTree();
    for (const [key, value] of this.nodes) copy.nodes.set(key, value.slice());
    return copy;
  }
