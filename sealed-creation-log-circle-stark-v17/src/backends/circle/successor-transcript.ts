...
export type SuccessorQueryOrbit = {
  /** Hash counter which first selected this previously unseen orbit. */
  readonly counter: number;
  readonly orbit: number;
  /** The hash-derived starting point inside the orbit. */
...
export class SuccessorTranscript {
  ...
  /** Canonical SHA-256 orbit selection without replacement. */
  queryOrbits(rowCount: number, count: number, orbitLog = 1): SuccessorQueryOrbit[] {
    ...
    const selected: SuccessorQueryOrbit[] = [];
    const orbits = new Set<number>();
    const orbitSize = 2 ** orbitLog;
    for (let counter = 0; selected.length < count; counter += 1) {
      ...
      const orbit = Math.floor(index / orbitSize);
      if (orbits.has(orbit)) continue;
      orbits.add(orbit);
      selected.push({ counter, orbit, seed: index & (orbitSize - 1) });
    }
    return selected;
  }

  /** Hash-derived representatives of unique fixed-size bit-reversed orbits. */
  queryIndices(rowCount: number, count: number, orbitLog = 1): number[] {
    const orbitSize = 2 ** orbitLog;
    return this.queryOrbits(rowCount, count, orbitLog).map(
      ({ orbit, seed }) => orbit * orbitSize + seed,
    );
  }
}
