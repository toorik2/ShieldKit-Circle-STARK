...
function syntheticCanonicalBytes(length = localWordMaximumCanonicalProofBytes(8)): Uint8Array {
...
describe("local-word byte-only proof carriers", () => {
  ...
  it("measures the exact unpadded carrier transaction below the consensus envelope", () => {
    ...
    assert.deepEqual(measurement, {
      ...
      carrierInputs: 210,
      carrierChunkMin: 453,
      carrierChunkMax: 3_707,
      budgetWeight: 56_580,
      unlockingSum: proofBytes.length + 3 * LOCAL_WORD_CARRIER_INPUTS,
      transactionBytes: 467_194,
      remainingConsensusBytes: 532_806,
    });
  });
});
