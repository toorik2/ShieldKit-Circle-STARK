...
function fullGraph() {
  ...
  const deposited = applyDeposit({
    state: emptyState(),
    poolCategory: new Uint8Array(32).fill(0x31),
...
describe("word-compressed local SHA copy permutation", () => {
  ...
  it("measures the complete withdrawal graph and keeps the named v16 worksheet floor", () => {
    ...
    assert.equal(geometry.activeWordSlots, 653_206);
    ...
    assert.equal(soundness.lookupTerms, 1_747_161);
    assert.equal(soundness.publicBoundaryTerms, 8);
    ...
    assert.equal(soundness.friQueryConjectureBits, 112);
