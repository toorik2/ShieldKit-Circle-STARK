...
export const REMAINING_PRODUCTION_WALLS = [
  ...
  'AIR q=90 cpi=3 blowup-8 merkle-stride-16 CM31 fold plus on-chain public-bind C/Z=FRI Q Libauth 15/15. Redeem 9913, unlocking 9405/8511/6263×15, tx 100000, op ≤7556196≤7556800. Residual on input 0, uniqueness on input 1. AIR LDE Merkle root absorbed before sampling zeta. HASH_NODE-compressed VERIFY_AIR_LDE leaves the 64-byte zeta row. Residual DEFINEs gated to input 0. Uniqueness and transcript replay on input 1; later inputs bind packed queries and fold-βs to input 1. Two-tier density pad. Later 4-to-1 fold-once+DUP. v6 skip-layer fold-only later headers. Round-0 π-pair Merkle; later large layers fold-only. Host fail-on-collision uniqueness; on-chain Fiat-Shamir rejection sampling. MULTIPROOF4 PICK1→OVER, ROLL1→SWAP, ROLL2→ROT. Protocol 1 λ absorbed as 0. HLP24 Thm 6 needs even k; k=45 odd so not instantiated. S_total union is unique-decoding (1/8)^45 plus HASH256 2^-256, floor 134 bits. Conjectural (2^17/M31^2)^45 is not the union. v2 not-qualified.',
