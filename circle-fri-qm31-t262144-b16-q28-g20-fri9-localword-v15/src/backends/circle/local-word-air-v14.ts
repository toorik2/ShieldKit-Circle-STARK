...
import type { LocalWordV14InteractionChallenges } from "./local-word-transcript-v14.ts";
import {
  LOCAL_SHA_LIMBS,
...
export const LOCAL_WORD_V14_BOUNDARY_PREPROCESSED_COLUMNS = 3;
export const LOCAL_WORD_V14_BOUNDARY_INTERACTION_QM31_COLUMNS = 2;
export const LOCAL_WORD_V14_PREPROCESSED_COLUMNS =
  LOCAL_SHA_WORD_PREPROCESSED_COLUMNS + LOCAL_WORD_V14_BOUNDARY_PREPROCESSED_COLUMNS;
export const LOCAL_WORD_V14_INTERACTION_QM31_COLUMNS =
  LOCAL_SHA_WORD_INTERACTION_QM31_COLUMNS + LOCAL_WORD_V14_BOUNDARY_INTERACTION_QM31_COLUMNS;
export const LOCAL_WORD_V14_AIR_CONSTRAINTS = LOCAL_SHA_WORD_AIR_CONSTRAINTS;
export const LOCAL_WORD_V14_AIR_PARTIAL_WIDTHS = [9, 8, 8] as const;

export type LocalWordV14AirFrame = {
...
function linear(
  ...
  if (challenges.length !== values.length) throw new Error("local-word v14 AIR linear arity");
  let result = gamma;
...
function compressedPort(
  frame: LocalWordV14AirFrame,
  challenges: LocalWordV14InteractionChallenges,
  port: "a" | "b" | "out",
): QM31El {
...
export function localWordV14AirResiduals(
  frame: LocalWordV14AirFrame,
  challenges: LocalWordV14InteractionChallenges,
  boundaryClaimedSum: QM31El,
): readonly QM31El[] {
  if (frame.original.length !== LOCAL_SHA_ORIGINAL_COLUMNS ||
    frame.preprocessed.length !== LOCAL_WORD_V14_PREPROCESSED_COLUMNS ||
    frame.interaction.length !== LOCAL_WORD_V14_INTERACTION_QM31_COLUMNS ||
    frame.interactionPrevious.length !== LOCAL_WORD_V14_INTERACTION_QM31_COLUMNS) {
    throw new Error("local-word v14 AIR frame geometry");
  }
  ...
  if (residuals.length !== LOCAL_WORD_V14_AIR_CONSTRAINTS) {
    throw new Error("local-word v14 AIR constraint count");
  }
  return residuals;
}

export function mixLocalWordV14AirResiduals(
  residuals: readonly QM31El[],
  alpha: QM31El,
): QM31El {
  if (residuals.length !== LOCAL_WORD_V14_AIR_CONSTRAINTS) throw new Error("local-word v14 AIR mix count");
  let result = QM31_ZERO;
...
export function localWordV14AirCompositionPartials(
  residuals: readonly QM31El[],
  alpha: QM31El,
): readonly [QM31El, QM31El, QM31El] {
  if (residuals.length !== LOCAL_WORD_V14_AIR_CONSTRAINTS ||
    LOCAL_WORD_V14_AIR_PARTIAL_WIDTHS.reduce((sum, width) => sum + width, 0) !== residuals.length) {
    throw new Error("local-word v14 AIR partial geometry");
  }
  let start = 0;
  return LOCAL_WORD_V14_AIR_PARTIAL_WIDTHS.map((width) => {
    let result = QM31_ZERO;
...
export function combineLocalWordV14AirCompositionPartials(
  ...
  return qmAdd(
    partials[0],
    qmAdd(
      qmMul(power(LOCAL_WORD_V14_AIR_PARTIAL_WIDTHS[0]), partials[1]),
      qmMul(
        power(LOCAL_WORD_V14_AIR_PARTIAL_WIDTHS[0] + LOCAL_WORD_V14_AIR_PARTIAL_WIDTHS[1]),
        partials[2],
      ),
    ),
  );
