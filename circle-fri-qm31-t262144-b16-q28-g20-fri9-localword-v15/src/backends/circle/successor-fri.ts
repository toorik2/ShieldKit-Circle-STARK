...
export function verifySuccessorFri(
  ...
  try {
    ...
    if (proof.layers.length !== foldCounts.length) {
      return { ok: false, reason: "FRI shape" };
    }
    if (proof.finalCoefficients.length !== 2 ** config.finalLogDegree) return { ok: false, reason: "FRI final width" };
    const alphas: QM31El[] = [];
    ...
    const expectedOpenings = openingIndices(originalQueries, foldCounts);
    let queryPositions = [...originalQueries];
    ...
    for (let round = 0; round < proof.layers.length; round += 1) {
      ...
      const rows = layer.indices.map((index, position) => ({ index, raw: encodeQm31(layer.values[position]!) }));
      ...
      if (foldedValues && queryPositions.some((position, query) => !qmEq(opened.get(position)!, foldedValues![query]!))) {
      ...
      foldedValues = queryPositions.map((position) => {
        ...
        for (let subfold = 0; subfold < folds; subfold += 1) {
          const challenge = subfold === 0 ? alphas[round]! : qmMul(alphas[round]!, alphas[round]!);
