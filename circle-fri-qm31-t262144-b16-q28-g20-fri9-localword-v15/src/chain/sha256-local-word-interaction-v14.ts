...
import type { LocalWordV14InteractionChallenges } from "../backends/circle/local-word-transcript-v14.ts";
import {
  LOCAL_SHA_LIMBS,
...
export type LocalShaV14InteractionTrace = {
...
function linear(
  ...
  if (challenges.length !== values.length) throw new Error("local SHA v14 lookup arity");
  let result = gamma;
...
function denominator(entry: LocalShaLookupEntry, challenges: LocalWordV14InteractionChallenges["lookup"]): QM31El {
...
export function buildLocalShaV14InteractionTrace(
  program: LocalShaProgram,
  trace: LocalShaTrace,
  challenges: LocalWordV14InteractionChallenges,
): LocalShaV14InteractionTrace {
  if (trace.rows.length !== program.rows.length) throw new Error("local SHA v14 interaction trace shape");
  ...
  if (accesses.length !== program.rows.length * LOCAL_SHA_LIMBS) {
    throw new Error("local SHA v14 lookup access geometry");
  }
  ...
  for (const entry of accesses) {
    ...
    if (index === undefined) throw new Error("local SHA v14 invalid lookup access");
    tableMultiplicities[index] = tableMultiplicities[index]! + 1n;
  }
  if (tableMultiplicities.some((value) => value >= M31)) {
    throw new Error("local SHA v14 table multiplicity M31");
  }
  ...
  if (!qmEq(lookupClaimedSum, QM31_ZERO)) throw new Error("local SHA v14 lookup claimed sum");
  ...
  if (columns.length !== LOCAL_SHA_WORD_INTERACTION_QM31_COLUMNS ||
    ...
    throw new Error("local SHA v14 interaction column geometry");
  }
