...
use crate::{
    canonical_tree_key, matrix_parent4, raw_leaf, relation_trace_lde, seal_relation_trace_lde,
    Qm31Value, SuccessorMatrixOpening, M31,
};
...
fn extend_column(values: &[u32], extension: DiskColumnExtension) -> Vec<u32> {
    let mut lde = match extension {
        DiskColumnExtension::SealedRelation { log_blowup } => {
            seal_relation_trace_lde(values, log_blowup)
        }
        DiskColumnExtension::PublicRelation { eval_log } => relation_trace_lde(values, eval_log),
        DiskColumnExtension::RandomPolynomial { eval_log } => relation_trace_lde(values, eval_log),
    };
...
fn build_parent_layers(
    ...
    if row_count < 4 || !row_count.is_power_of_two() || row_count.ilog2() % 2 != 0 {
        return Err("disk oracle Merkle-4 rows".into());
    }
    ...
    while nodes > 1 {
        ...
        for start in (0..parents).step_by(NODE_CHUNK) {
            ...
            let mut bytes = vec![0u8; count * 128];
            reader
            ...
            let hashes: Vec<[u8; 32]> = bytes
                ...
                .map(|group| {
                    let children: [[u8; 32]; 4] = std::array::from_fn(|child| {
                        group[child * 32..(child + 1) * 32].try_into().unwrap()
                    });
                    matrix_parent4(&tree_key, level as u8, &children)
                })
                .collect();
            for hash in hashes {
                writer
...
pub fn build_disk_matrix_commitment(
    ...
    for (column, values) in relation_columns.iter().enumerate() {
        let lde = extend_column(values, extension);
        if let Some(expected) = row_count {
            if lde.len() != expected {
                return Err("disk oracle LDE rows".into());
            }
        } else {
            row_count = Some(lde.len());
        }
        write_u32_column(&column_path(directory, column), &lde)?;
    }
    let row_count = row_count.unwrap();
    if row_count < 4 || row_count.ilog2() % 2 != 0 {
        return Err("disk oracle Merkle-4 rows".into());
    }
    build_leaf_layer(label, directory, row_count, relation_columns.len())?;
    let root = build_parent_layers(label, directory, row_count)?;
    Ok(DiskMatrixCommitment {
        ...
        root,
        layer_count: row_count.ilog2() as usize / 2 + 1,
    })
}
...
pub fn combine_disk_matrix_commitments(
    ...
    build_leaf_layer(label, directory, row_count, target_column)?;
    let root = build_parent_layers(label, directory, row_count)?;
    Ok(DiskMatrixCommitment {
        label: label.to_string(),
        directory: directory.to_path_buf(),
...
pub fn commit_disk_qm31_evaluation(
    ...
    if label.is_empty()
        || directory.exists()
        || evaluation.row_count < 4
        || !evaluation.row_count.is_power_of_two()
        || evaluation.row_count.ilog2() % 2 != 0
    {
    ...
    build_leaf_layer(label, directory, evaluation.row_count, 4)?;
    let root = build_parent_layers(label, directory, evaluation.row_count)?;
    Ok(DiskMatrixCommitment {
        label: label.to_string(),
        directory: directory.to_path_buf(),
        ...
        column_count: 4,
        root,
        layer_count: evaluation.row_count.ilog2() as usize / 2 + 1,
    })
}
