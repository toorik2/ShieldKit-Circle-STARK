...
import {
  ...
  compileLocalWordCompositionQuotientGate,
...
const QUERY = 777_710;
const QMASK_ROWS = 100_000;
const ALPHA = qm31(5n, 7n, 11n, 13n);
const PARTIALS = [
  qm31(17n, 19n, 23n, 29n),
  qm31(31n, 37n, 41n, 43n),
  qm31(47n, 53n, 59n, 61n),
] as const;
const FRI_MASK = qm31(67n, 71n, 73n, 79n);

function proofFixture(): { readonly proof: Uint8Array; readonly partialOffset: number } {
  const offsets = localWordProofStaticOffsets(18);
  ...
  proof.set(writeU32BE(QMASK_ROWS), offsets.openingDirectory + qmask * 20 + 4);
  PARTIALS.forEach((partial, part) =>
    proof.set(encodeQm31(partial), offsets.compositionPartials + part * 16));

  const composition = combineLocalWordAirCompositionPartials(PARTIALS, ALPHA);
  const zerofier = localWordRelationZerofierAtBitReversed(QUERY);
  const quotient = qmMulM31(composition, mInv(zerofier));
  proof.set(encodeQm31(quotient), QMASK_ROWS);
  proof.set(encodeQm31(FRI_MASK), QMASK_ROWS + 16);
  return { proof, partialOffset: offsets.compositionPartials };
}
...
describe("local-word composition and quotient on the May-2026 VM", () => {
  it("binds the three checked AIR partials to the authenticated sealed-trace quotient", () => {
    const built = proofFixture();
    const lock = compileLocalWordCompositionQuotientGate({ profile: 0, query: 0 });
    const honest = evaluate(built.proof, lock);
    assert.equal(honest.vm.stateSuccess(honest.state), true, String(honest.state.error));

    const offsets = localWordProofStaticOffsets(18);
    const mutations: readonly [string, Uint8Array][] = [
      ["alpha", changed(built.proof, offsets.constraintAlpha)],
      ["partial", changed(built.proof, built.partialOffset)],
      ["quotient", changed(built.proof, QMASK_ROWS)],
      ["zerofier orbit", (() => {
        const proof = built.proof.slice();
        proof.set(writeU32BE(QUERY + 2 ** 18), offsets.queries);
        return proof;
      })()],
      ["noncanonical partial", (() => {
        const proof = built.proof.slice();
        proof.set(writeU32LE(2_147_483_647), built.partialOffset);
        return proof;
      })()],
    ];
    for (const [label, mutation] of mutations) {
      const rejected = evaluate(mutation, lock);
    ...
    console.log("local-word-composition-quotient-gate", JSON.stringify({
...
function batchFixture(): {
  ...
  const composition = combineLocalWordAirCompositionPartials(PARTIALS, ALPHA);
  const quotient = qmMulM31(
    composition,
    mInv(localWordRelationZerofierAtBitReversed(QUERY)),
  );
...
describe("local-word shared quotient and batching algebra on the May-2026 VM", () => {
  it("deletes duplicate QM31 setup while preserving both deterministic checks", () => {
    const built = batchFixture();
    const lock = compileLocalWordCompositionBatchGate({ profile: 0, query: 0 });
    const honest = evaluate(built.proof, lock);
    assert.equal(honest.vm.stateSuccess(honest.state), true, String(honest.state.error));
    const offsets = localWordProofStaticOffsets(18);
    for (const [label, offset] of [
      ["alpha", offsets.constraintAlpha],
      ["partial", offsets.compositionPartials],
      ...built.mutations,
    ...
    const operationCost = Number(honest.state.metrics.operationCost);
    console.log("local-word-composition-batch-gate", JSON.stringify({
      lockingBytes: lock.length,
      operationCost,
      requiredChunkBytes: localWordRequiredChunkBytes(operationCost),
    }));
...
describe("local-word FRI algebra on the May-2026 VM", () => {
  it("checks all 17 folds and the final degree-7 polynomial for one query", () => {
    ...
    noncanonical.set(writeU32LE(2_147_483_647), localWordProofStaticOffsets(18).finalCoefficients);
