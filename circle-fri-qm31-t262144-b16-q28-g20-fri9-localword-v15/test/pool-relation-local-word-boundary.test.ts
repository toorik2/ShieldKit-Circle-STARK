...
function fixture() {
  ...
  const deposited = applyDeposit({
    state: emptyState(new Uint8Array(32).fill(0x51)),
...
describe("local-word public statement boundary", () => {
  it("binds the 34 settlement-owned words with two quadratic LogUp columns", () => {
    ...
    assert.equal(soundness.boundaryTerms, 2359296);
